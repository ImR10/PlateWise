...
# Money/measurement precision for nutrition and recipe quantities.
...
class MenuItem(UUIDPrimaryKeyMixin, TimestampMixin, SourceTrackingMixin, Base):
    ...
    __table_args__ = (
        # UNIQUE(institution_id, external_id) *only* where an external id exists;
        # multiple NULL external ids per institution are allowed.
        Index(
            "uq_menu_items_institution_id_external_id",
            "institution_id",
            "external_id",
            unique=True,
    ...
    default_serving_size: Mapped[float | None] = mapped_column(_QUANTITY, nullable=True)
    ...
    @property
    def current_nutrition(self) -> NutritionFacts | None:
        """Return the single active nutrition record (``valid_until IS NULL``)."""
        for facts in self.nutrition_facts:
            if facts.valid_until is None:
                return facts
        return None
...
class NutritionFacts(UUIDPrimaryKeyMixin, TimestampMixin, SourceTrackingMixin, Base):
    ...
    At most one *active* record (``valid_until IS NULL``) exists per menu item;
    superseded versions are retained for history.
    """
    ...
    __table_args__ = (
        ...
        # Enforce a single active nutrition record per menu item.
        Index(
            "uq_nutrition_facts_active_per_menu_item",
            "menu_item_id",
            unique=True,
    ...
    serving_size: Mapped[float | None] = mapped_column(_QUANTITY, nullable=True)
    serving_unit: Mapped[str | None] = mapped_column(String(50), nullable=True)

    calories: Mapped[float | None] = mapped_column(_NUTRIENT, nullable=True)
    protein_g: Mapped[float | None] = mapped_column(_NUTRIENT, nullable=True)
    carbohydrates_g: Mapped[float | None] = mapped_column(_NUTRIENT, nullable=True)
    fat_g: Mapped[float | None] = mapped_column(_NUTRIENT, nullable=True)
    saturated_fat_g: Mapped[float | None] = mapped_column(_NUTRIENT, nullable=True)
    fiber_g: Mapped[float | None] = mapped_column(_NUTRIENT, nullable=True)
    sugar_g: Mapped[float | None] = mapped_column(_NUTRIENT, nullable=True)
    sodium_mg: Mapped[float | None] = mapped_column(_NUTRIENT, nullable=True)
    cholesterol_mg: Mapped[float | None] = mapped_column(_NUTRIENT, nullable=True)
...
class MenuItemIngredient(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    quantity: Mapped[float | None] = mapped_column(_QUANTITY, nullable=True)
...
class DietaryTag(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    __table_args__ = (
        UniqueConstraint("normalized_name", name="uq_dietary_tags_normalized_name"),
    )
