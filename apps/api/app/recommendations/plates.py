...
from app.imports.decimal_utils import quantize_nutrient
...
from app.recommendations.enums import ResultWarning
...
def _totals(
    ...
    for field in NUTRIENT_FIELDS:
        values = [
            item.nutrition.nutrients if item.nutrition is not None else None
            for item in selected
        ]
        nutrient_values = [
            getattr(nutrients, field) if nutrients is not None else None
            for nutrients in values
        ]
...
def assemble_plate(
    ...
    if low is None and high is None:
        ...
        explanation = (
            "Combines the top-ranked eligible items; no calorie target was set."
        )
    else:
