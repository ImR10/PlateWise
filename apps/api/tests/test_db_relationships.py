...
from app.db.enums import AllergenDeclarationType, MealPeriod, ReportType
from app.db.models import MenuItem, MenuOffering
...
def test_menu_item_allergens(db_session: Session) -> None:
    ...
    f.link_allergen(
        db_session, item, peanut, declaration_type=AllergenDeclarationType.MAY_CONTAIN
    )
...
def test_import_tracks_created_records(db_session: Session) -> None:
    ...
    offering = f.make_offering(
        db_session, station, item, created_by_import_id=data_import.id
    )
