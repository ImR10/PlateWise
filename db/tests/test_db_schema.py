...
import platewise_db.pytest_plugin as db_test_support
...
def test_migration_upgrade_downgrade_roundtrip(database_url: str) -> None:
    ...
    db_test_support._ensure_database_exists(scratch_url)
    scratch_engine = create_engine(scratch_url)
    try:
        cfg = db_test_support._alembic_config(scratch_url)
        ...
        maint = create_engine(base_url.set(database="postgres"), isolation_level="AUTOCOMMIT")
        try:
