...
from platewise_db.decimal_utils import quantize_grams, quantize_quantity, quantize_serving
...
from platewise_db.normalizers import content_hash, normalize_name
...
def upsert_institution(session: Session, imported: Any) -> tuple[Institution, Outcome]:
    digest = content_hash(
...
def upsert_venue(
    session: Session, institution: Institution, imported: Any
) -> tuple[Venue, Outcome]:
    ...
    if existing is None:
        existing = session.scalar(
            select(Venue).where(Venue.institution_id == institution.id, Venue.slug == imported.slug)
        )
    if existing is None:
        venue = Venue(
...
def upsert_station(session: Session, venue: Venue, imported: Any) -> tuple[Station, Outcome]:
...
def upsert_provider_food(session: Session, food: Any) -> ProviderFood:
    existing = session.scalar(
...
def recipe_content_hash(recipe: Any) -> str:
    parts: list[object] = [
...
def get_active_calculated_nutrition(session: Session, menu_item: MenuItem) -> NutritionFacts | None:
