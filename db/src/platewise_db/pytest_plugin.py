...
Database tests fail if PostgreSQL is unreachable; a broken persistence test
environment must never silently reduce suite coverage.
"""
...
from platewise_db.session import get_database_url
...
def _test_database_url() -> str:
    ...
    if explicit:
        return explicit
    base = os.getenv("DATABASE_URL", get_database_url())
    url = make_url(base)
...
def _alembic_config(url_str: str) -> Config:
    ...
    config.set_main_option("script_location", str(DB_ROOT / "alembic"))
