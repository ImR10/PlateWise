...
from platewise_api.recommendations.enums import ResultWarning, SafetyMode
...
def test_completion_log_event_is_structured_and_safe(
    ...
    with caplog.at_level(logging.INFO, logger="platewise_api.recommendations.service"):
        recommend(items, fx.prefs(excluded_allergens=("peanut",), safety_mode="permissive"))
    records = [r for r in caplog.records if r.getMessage() == "recommendation_run_completed"]
...
def test_plate_totals_never_zero_fill_missing_nutrients() -> None:
    ...
    result = recommend(items, fx.prefs(assemble_plate=True, calorie_target=Decimal("700")))
    plate = result.plate
    assert plate is not None
...
def test_plate_not_assembled_when_no_item_fits_budget() -> None:
    ...
    items = [fx.make_item("a", "Apple", nutrition=fx.make_nutrition(calories=None, protein_g="5"))]
    result = recommend(
        items,
        fx.prefs(
            assemble_plate=True,
