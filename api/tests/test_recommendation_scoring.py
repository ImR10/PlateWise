...
from platewise_db.enums import NutritionProvenance, OfferingStatus

from platewise_api.recommendations.contracts import RecommendationItem
...
from platewise_api.recommendations.scoring import (
    GOAL_WEIGHTS,
...
def test_lower_calorie_item_scores_higher_for_lower_calorie_goal() -> None:
    ...
    assert _score(light, goal=GoalType.LOWER_CALORIE) > _score(heavy, goal=GoalType.LOWER_CALORIE)
...
def test_balanced_goal_does_not_over_reward_one_nutrient() -> None:
    protein_monster = fx.make_item(
        "monster",
        nutrition=fx.make_nutrition(calories="500", protein_g="60", fiber_g="0", sodium_mg="1400"),
    )
    moderate = fx.make_item(
        "moderate",
        nutrition=fx.make_nutrition(calories="400", protein_g="15", fiber_g="8", sodium_mg="200"),
    )
...
def test_scores_are_bounded_and_deterministic() -> None:
    best = fx.make_item(
        "best",
        nutrition=fx.make_nutrition(calories="0", protein_g="99", fiber_g="99", sodium_mg="0"),
    )
    worst = fx.make_item(
        "worst",
        nutrition=fx.make_nutrition(calories="5000", protein_g="0", fiber_g="0", sodium_mg="9000"),
    )
    for goal in GoalType:
...
def test_calorie_fit_target_and_range() -> None:
    ...
    over_fit = {c.component: c.score for c in over_breakdown.components}[ScoreComponent.CALORIE_FIT]
