...
from platewise_api.recommendations.explanations import (
    CAUTION_TEXT,
    EXCLUSION_TEXT,
    POSITIVE_TEXT,
    WARNING_TEXT,
...
from platewise_api.recommendations.scoring import score_item
...
def test_reason_codes_are_machine_readable_and_stable() -> None:
    ...
    assert ExclusionReason.INSUFFICIENT_NUTRITION_DATA.value == "INSUFFICIENT_NUTRITION_DATA"
...
def test_human_readable_explanations_are_stable() -> None:
    ...
    assert explain_cautions((CautionReason.SERVING_SIZE_UNKNOWN,)) == ("Serving size is unknown.",)
...
def test_positive_reasons_match_actual_score_components() -> None:
    item = fx.make_item(
        nutrition=fx.make_nutrition(calories="450", protein_g="40", fiber_g="8", sodium_mg="150")
    )
