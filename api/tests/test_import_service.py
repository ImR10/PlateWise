...
import platewise_db.repositories.imports as repo
import pytest
...
def test_recipe_import_calculates_and_persists(db_session: Session) -> None:
    ...
    rice = db_session.scalar(select(RecipeIngredient).where(RecipeIngredient.line_no == 3))
...
def test_import_lifecycle_logs_stable_fields_without_raw_payload(
    ...
    caplog.set_level("INFO", logger="platewise_api.imports.service")
