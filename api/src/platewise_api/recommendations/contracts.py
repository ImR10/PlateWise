...
Conventions shared with the import pipeline (`platewise_api.imports.contracts`):
...
TargetDecimal = Annotated[Decimal, Field(gt=0, le=Decimal("99999999.99"), allow_inf_nan=False)]
...
class RecommendationItem(_Contract):
    ...
    dietary_tags: tuple[DietaryTagInfo, ...] = Field(default=(), max_length=MAX_ITEM_DIETARY_TAGS)
...
class UserPreferences(_Contract):
    ...
    excluded_allergens: tuple[str, ...] = Field(default=(), max_length=MAX_PREFERENCE_LIST_LENGTH)
    ...
    disliked_item_ids: tuple[str, ...] = Field(default=(), max_length=MAX_PREFERENCE_LIST_LENGTH)
    disliked_item_names: tuple[str, ...] = Field(default=(), max_length=MAX_PREFERENCE_LIST_LENGTH)

    max_results: int = Field(default=DEFAULT_MAX_RESULTS, ge=1, le=MAX_RESULTS_LIMIT)
    #: Opt-in basic plate assembly (see ``platewise_api.recommendations.plates``).
