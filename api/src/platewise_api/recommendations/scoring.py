...
from platewise_db.decimal_utils import ROUNDING
from platewise_db.enums import NutritionProvenance, NutritionReviewStatus, OfferingStatus
...
def protein_adequacy(protein_g: Decimal | None, preferences: UserPreferences) -> Decimal | None:
...
def protein_density(protein_g: Decimal | None, calories: Decimal | None) -> Decimal | None:
...
def compute_confidence(item: RecommendationItem) -> Decimal:
    ...
    confidence *= AVAILABILITY_CONFIDENCE.get(item.availability, UNCERTAIN_AVAILABILITY_CONFIDENCE)
...
def _data_cautions(item: RecommendationItem) -> list[CautionReason]:
    ...
    if not nutrition.is_complete or nutrition.review_status is NutritionReviewStatus.NEEDS_REVIEW:
        cautions.append(CautionReason.NUTRITION_INCOMPLETE)
    if any(getattr(nutrition.nutrients, field) is None for field in CORE_NUTRIENT_FIELDS):
